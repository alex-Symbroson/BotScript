...
namespace Variables {
    ...
    void _stringify(
        var value, string* str, bool mark = true, bool expand_func = false);
...
int main() {
    ...
    cout << *list->at(0)->getStr() << getStr(list->at(1)) << getInt(list->at(2))
<< endl; *list += list; //list->getLst()->insert(list->getLst()->end(),
list->getLst()->begin(), list->getLst()->end());
